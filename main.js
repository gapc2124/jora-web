...
/* =========================
   Carrusel Principal (Efecto BARRIDO / SLIDE) (Actualizado)
   ========================= */
(function carousel() {
    const root = $('.carousel');
    if (!root) return;

    // Seleccionamos el contenedor que se moverá (.slides)
    ...
    // Función principal para MOVER el carrusel (Barrido)
    function show(idx) {
        // Lógica circular (loop infinito)
        i = (idx + slides.length) % slides.length;
        
        // Aplicamos la transformación CSS para desplazar el track
        // Multiplicamos el índice por -100% para mover a la izquierda
        track.style.transform = `translateX(-${i * 100}%)`;

        // Actualizamos estado de los dots
        if (dots.length) {
            dots.forEach((d, k) => {
    ...
    dots.forEach((d, k) => d.addEventListener('click', () => {
        stopAuto(); // Detener si el usuario interactúa
        show(k);
    }));
